...
correct and investigate. Make sure to glance at the data before
cleaning.
...
data, or use the cleaned data if it's already loaded. \[read in the
species aggregated abundance\]
...
abund_traits <- left_join(traits_agg_sp, abund, by = "final_name")
head(abund_traits)
```
...
species' body size to calculate the first three Hill numbers (0, 1, 2)
...
Next, calculate Hill 0, 1, and 2 inside the `summarize()` function.

```{r agg-traits-site-soln, eval=FALSE}
at_agg_site <- summarize(at_grouped_site,
                         hill_0 = hill_calc(mean_mass_g, 0),
...
# total
plot(density(abund_traits$mean_mass_g), xlab = "Average mass (g)", ylab = "Density", main = "All species")
```
...
It appears that `Kauai_01` is widely dispersed with an average that may
be a bit higher than the other islands. What might this suggest about
the community assembly mechanism?
...
Kauai and Maui have more even trait distributions, while Hawaii has a
less even trait distribution.

```{r ranked-site, eval=FALSE}
plot(
    sort(abund_traits_split$HawaiiIsland_01$mean_mass_g, decreasing = TRUE),
    main = "Average mass per study site",
    xlab = "Rank",
    ylab = "Average mass (mg)",
    pwd = 2,
...
points(
    sort(abund_traits_split$Kauai_01$mean_mass_g, decreasing = TRUE),
    pwd = 2,
    pch = 19,
    col = "#31688EFF"
)
points(
    sort(abund_traits_split$Maui_01$mean_mass_g, decreasing = TRUE),
    pwd = 2,
    pch = 19,
    col = "#35B779FF"
)
