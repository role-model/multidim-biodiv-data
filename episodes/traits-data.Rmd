...
with the path to the data. Take a glance at the data with the `head()`
function. `str()` is another useful function to look at the structure of
the data. The data contains two columns- `GenSp` which contains species
binomial names and `mass_mg`, which contains the mass of the individual
in milligrams.
...
```{r trait-import-sol-hidden, eval=FALSE}
traits <- read.csv('episodes/data/abund.csv')
