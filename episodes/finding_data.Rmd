...
For GBIF access using the spocc package, we use the occ() function where a species name can be put as the query 
...
For NCBI access using the rentez package we can also search for a species, but unlike spocc this returns a set of record IDs rather than the record information. Other databases also use this kind of pipeline. 
...
OTOL works similarly, but the first search returns a set of resolved taxon IDs organized in a dataframe, not record IDs like NCBI.
...
In addition to or instead of querying by species, we can search for records in a given subdatabase or location in the world 
