...
```{r load data}
abundances <- read.csv("https://github.com/role-model/multidim-biodiv-data/raw/abundance-edits/episodes/data/long_abundances.csv")
...
wtf <- taxize::gnr_resolve(species_list)
...
abundances <- left_join(abundances, best_scores, by = c("GenSp" = "user_supplied_name"))

abundances[ which(abundances$GenSp != abundances$matched_name), ]

```

::: discussion

`taxize` is not a magic wand! alas.

specifically, for example, names with no matches will be _dropped_.
...
abundance_tallies <- group_by(abundances, site, island, matched_name)
abundance_tallies <- summarize(abundance_tallies, abundance = dplyr::n())
...
```{r or base r}
x <- aggregate(abundances[, 'matched_name', drop = FALSE], abundances[, c('matched_name', 'site')], 
               length)
